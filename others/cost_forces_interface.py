...
def cartpole_simulator1(z, p):
    ...
    return -casadi.cos(z[1]) + -0.1*casadi.cos(z[2]) #+ -0.01*casadi.cos(z[4])#+ cartpole_position_weight*(z[3] - p[3])**2
...
def dubins_obstacles():
    return [[0.31868297, 0.7749577, 0.28699964], [-0.49458456, -0.11240232, 0.27588597], [0.19568837, -0.5972406, 0.082922354], [0.06757867, -0.42966557, 0.2968905], [0.6409783, 0.36124045, 0.29200518], [-0.52833635, 0.17657721, 0.19466914], [0.36637014, -0.600214, 0.20858128], [-0.040501416, -0.16123867, 0.10538994], [0.39486748, -0.2967993, 0.23954614], [-0.47237647, 0.27628905, 0.13882904], [-0.49906766, 0.5697846, 0.1728635], [-0.61574525, 0.73863643, 0.14459442], [0.41435724, 0.4362622, 0.05848365]]
...
def dubins_car(z, p):
    ...
    for i in range(0, obstacles.shape[0]):
        ...
        # d = casadi.norm_2(z[3:6] - obst[i:i+3])
        d = casadi.sumsqr(z[2:4] - obstacles[i, :-1])
        if radius> 0.20:
            obstacles_cost += 1.0 - (casadi.fmin(1.0, 1.0*d / radius))
