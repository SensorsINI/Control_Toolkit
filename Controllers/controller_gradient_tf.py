...
class controller_gradient_tf(template_controller):
    ...
    def step(self, s: np.ndarray, time=None):
        ...
        for _ in range(0, self.gradient_steps):
            ...
            with tf.GradientTape(watch_accessed_variables=False) as tape:
                tape.watch(self.Q_tf)
                traj_cost, rollout_trajectory = self.predict_and_cost(s, self.Q_tf)
            #retrieve gradient of cost w.r.t. input sequence
            dc_dQ = tape.gradient(traj_cost, self.Q_tf)
            ...
            self.optim.apply_gradients(zip([dc_dQ_prc], [self.Q_tf]))
            # clip
            Q = tf.clip_by_value(self.Q_tf, self.action_low, self.action_high) 
