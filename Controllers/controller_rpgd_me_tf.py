...
class controller_rpgd_me_tf(template_controller):
    ...
    def grad_step(
        ...
        dc_dQ = tape.gradient(traj_cost_mc_estimate, theta)
        dc_dQ_prc = tf.clip_by_norm(dc_dQ, self.gradmax_clip)
        # use optimizer to apply gradients and retrieve next set of input sequences
        opt.apply_gradients(zip([dc_dQ_prc], [theta]))
        # clip
    ...
    def get_action(self, s: tf.Tensor, epsilon: tf.Tensor, theta: tf.Variable):
        ...
        return u, epsilon_new, best_idx, traj_cost, rollout_trajectory
    ...
    def step(self, s: np.ndarray, time=None):
        ...
        for _ in range(0, iters):
            theta, traj_cost = self.grad_step(s, self.epsilon, self.theta, self.opt)
        ...
        if self.controller_logging:
            self.current_log["Q_logged"] = self.zeta(self.theta, epsilon_new).numpy()
        ...
        if self.count % self.resamp_per == 0:
            ...
            epsilon_keep = tf.gather(epsilon_new, best_idx, axis=0)  # resorting according to costs
            ...
            if len(adam_weights) > 0:
                w1 = tf.concat(
                    [
                        adam_weights[1][:, 1:, :],
                        tf.zeros([1, 1, self.theta.shape[-1]]),
                    ],
                    axis=1,
                )
                w2 = tf.concat(
    ...
    def controller_reset(self):
        ...
        initial_theta = np.tile(initial_theta, (self.mpc_horizon, 1))[np.newaxis]
